...
const sounds = {
  SHOOT: new Audio(
    "https://freesound.org/data/previews/459/459145_6142149-lq.mp3"
  ),
  EXPLODE: new Audio(
    "https://audiosoundclips.com/wp-content/uploads/2019/10/8-Bit-SFX_Explosion-2.mp3"
  ),
  MISSILE: new Audio(
    "https://freesound.org/data/previews/404/404754_140737-lq.mp3"
  ),
  BOOST: new Audio(
    "https://freesound.org/data/previews/340/340956_5858296-lq.mp3"
  ),
  HIT: new Audio(
    "https://freesound.org/data/previews/386/386862_6891102-lq.mp3"
  ),
  SHIELD: new Audio(
  ...
  LIFE: new Audio(
    "https://freesound.org/data/previews/397/397743_3905081-lq.mp3"
  ),
  VICTORY: new Audio(
    "https://freesound.org/data/previews/138/138485_758593-lq.mp3"
  ),
  LOSE: new Audio(
...
function randomColor() {
  ...
  return [
    min[0] + Math.random() * 200,
    min[1] + Math.random() * 200,
    min[2] + Math.random() * 200
  ];
}
...
function newWave() {
  ...
  teams++;
  let bosses = wave % 5 === 0 ? wave / 5 : 0;
  let colors = [];
  for (s = 1; s <= teamSize; s++) {
    for (t = 1; t <= teams; t++) {
      if (t === ship.team && s === teamSize) {
        continue;
      }
      if (colors.length < t) {
        colors.push(t === ship.team ? ship.color : randomColor());
      }
      let isBoss = wave % 5 === 0 && bosses > 0 && t !== ship.team;
      genShip(t, colors[t - 1], isBoss);
      if (isBoss) {
        bosses--;
      }
    }
...
function keys() {
  if (ship.lives === 0) {
    if (keyIsDown(keyCode) && new Date() - deathTime > 500) {
      reset();
    }
    return;
  }
  if (keyIsDown(UP_ARROW) || keyIsDown(87)) {
    if (currentTime - lastKeyTime.up <= delta && !lastKey.up) {
      if (currentTime - ship.wait.boost >= boostWait) {
        boost(ship, "Up");
      }
    } else {
...
function refresh() {
  if (!newGame && ship.lives > 0 && enemies === 0) {
    newWave();
  }
  if (totalShips < maxShips && shipQueue.length > 0) {
    pushShip(shipQueue.pop());
  }
  drawStars();
  flameFlicker();
...
function drawUI() {
  ...
  if (ship.lives === 0) {
    writeText(
      ...
      255,
      255,
      Math.min(new Date() - deathTime, 255),
      "You died on " +
        (wave === 0 ? "the tutorial" : "wave " + wave) +
        ". You had " +
        ship.kills +
        " kill" +
        (ship.kills === 1 ? "" : "s") +
    ...
    if (gap < textTime) {
      writeText(
        ...
        255,
        255,
        Math.min(gap, textTime - gap),
        "Wave " + wave + ": Defeat " + enemies + " enemies."
      );
    }
  }
  if (!newGame) {
...
function tutorial() {
  ...
  if (gap < textTime) {
    ...
    writeText(
      ...
      255,
      255,
      Math.min(gap, textTime * 2 - gap),
      "Press [↑] or [W] to accelerate and [←] [→] or [A] [D] to rotate"
    );
    ...
    writeText(
      ...
      255,
      255,
      Math.min(gap, textTime * 3 - gap),
      "Press [Space] or [I] to shoot"
    );
    ...
    writeText(
      ...
      255,
      255,
      Math.min(gap, textTime * 4 - gap),
      "Double-tap [↑] [←] [→] or [W] [A] [D] to boost"
    );
    ...
    writeText(
      ...
      255,
      255,
      Math.min(gap, textTime * 5 - gap),
      "Double-tap [Space] or [I] to fire a missile"
    );
    ...
    writeText(
      ...
      255,
      255,
      Math.min(gap, textTime * 6 - gap),
      "Defeat enemies, collect shields, and avoid planets"
    );
  } else {
    newGame = false;
    generation();
...
function outOfBounds(object) {
  return (
    object.xPos > bounds + ship.xPos + object.size / 2 ||
    object.xPos < -bounds + ship.xPos - object.size / 2 ||
    object.yPos > bounds + ship.yPos + object.size / 2 ||
    object.yPos < -bounds + ship.yPos - object.size / 2
  );
}
...
function offScreen(object) {
  if (object.type === types.PLANET && object.ring) {
    return (
      object.xPos > width / 2 + ship.xPos + object.size ||
      object.xPos < -width / 2 + ship.xPos - object.size ||
      object.yPos > height / 2 + ship.yPos + object.size / 2 ||
      object.yPos < -height / 2 + ship.yPos - object.size / 2
    );
  }
  return (
    object.xPos > width / 2 + ship.xPos + object.size / 2 ||
    object.xPos < -width / 2 + ship.xPos - object.size / 2 ||
    object.yPos > height / 2 + ship.yPos + object.size / 2 ||
    object.yPos < -height / 2 + ship.yPos - object.size / 2
  );
...
function regenRange(object) {
  return (
    object.xPos > width + ship.xPos + object.size / 2 ||
    object.xPos < -width + ship.xPos - object.size / 2 ||
    object.yPos > height + ship.yPos + object.size / 2 ||
    object.yPos < -height + ship.yPos - object.size / 2
  );
}
...
function getDistance(a, b) {
  return (
    Math.sqrt(Math.pow(a.xPos - b.xPos, 2) + Math.pow(a.yPos - b.yPos, 2)) -
    a.size / 2 -
    b.size / 2
  );
}
...
function adjust(object) {
  if (object.type === types.SHIP) {
    adjustShip(object);
  } else if (object.type === types.BULLET || object.type === types.DEBRIS) {
    adjustParticle(object);
  } else if (outOfBounds(object)) {
    object.end = true;
  } else if (object.type === types.EFFECT) {
    adjustEffect(object);
  }
}

function adjustShip(object) {
  if (object !== ship && outOfBounds(object)) {
    genCoords(object);
    object.target = ship;
  }
...
function adjustEffect(object) {
  if (object.fade > 0) {
    ...
    if (object.effect === effects.EXPLOSION) {
      object.size += 7;
    }
  } else {
    object.end = true;
...
function collide(a, b) {
  if (a.end || b.end || a.type === types.EFFECT || b.type === types.EFFECT) {
    return;
  }
  ...
  if (resultA.end) {
    ...
    if (a.type === types.SHIP) {
      ...
      if (b.type == types.BULLET && b.parent != null) {
        b.parent.kills++;
      }
    }
  }
  if (resultB.end) {
    ...
    if (b.type === types.SHIP) {
      ...
      if (a.type == types.BULLET && a.parent != null) {
        a.parent.kills++;
      }
    }
  }
...
function bump(a, b) {
  ...
  if (b.type === types.DEBRIS) {
    return result;
  } else if (b.type === types.PLANET) {
    a.xVel = 0;
    a.yVel = 0;
    a.dVel = 0;
  }
...
function onTeam(a, b) {
  ...
  let aTeam, bTeam;
  if (typeof(a.team) === "undefined" || a.team === 0) {
    return false;
  } else {
    aTeam = a.team;
  }
  if (typeof(b.team) === "undefined" || b.team === 0) {
    return false;
  } else {
    bTeam = b.team;
  }
...
function endObject(i) {
  if (
    objects[i].type === types.SHIP ||
    (objects[i].type === types.BULLET && objects[i].missile) ||
    objects[i].type === types.LOOT
  ) {
...
function isShield(object) {
  return (
    currentTime - object.time.shield <
    shieldTime / (object !== ship ? CPUwait : 1)
  );
}
...
function trackTarget(object) {
  ...
  if (
    object.target.type === types.PLANET ||
    (object.type === types.SHIP && object.target.type === types.BULLET)
  ) {
    direction += !inFront(object, object.target)
      ? onRight(object, object.target)
        ? -PI / 3
        : PI / 3
      : directFront(object, object.target)
      ? PI
      : onRight(object, object.target)
      ? -PI / 2
      : PI / 2;
  ...
  if (object.target.type === types.SHIP) {
    ...
    if (
      ...
      object.type !== types.BULLET
    ) {
      speed = Math.min(speed, Math.max(0, velDifference));
    }
  }
  if (speed > 0) {
...
function attackTarget(a, b) {
  ...
  currentTime = new Date();
  if (
    distance < a.size * CPUgap &&
    currentTime - a.wait.boost >= boostWait * CPUwait
  ) {
    boost(a, directFront(a, b) ? "Up" : onRight(a, b) ? "Right" : "Left");
...
function calcGravity(a, b) {
  if (a.type === types.PLANET && b.type === types.PLANET) {
    return;
  }
  if (a.type !== types.PLANET) {
    if (b.type === types.PLANET) {
...
function genCoords(object) {
  ...
  object.xPos =
    (Math.sign(Math.random() - 0.5) *
      (Math.random() * bounds + (width + getGravSize(object)) * xOrY)) /
      2 +
    ship.xPos;
  object.yPos =
    (Math.sign(Math.random() - 0.5) *
      (Math.random() * bounds + (height + getGravSize(object)) * (1 - xOrY))) /
      2 +
    ship.yPos;
...
function genStarCoords(object) {
  ...
  object.xPos =
    ((Math.sign(Math.random() - 0.5) *
      (Math.random() * width + (width + size) * xOrY)) /
      2) *
      object.layer +
    ship.xPos;
  object.yPos =
    ((Math.sign(Math.random() - 0.5) *
      (Math.random() * height + (height + size) * (1 - xOrY))) /
      2) *
      object.layer +
    ship.yPos;
}
...
function genShip(team = 0, color = randomColor(), boss = false) {
  ...
  newShip.team === ship.team ? allies++ : enemies++;
  shipQueue.push(newShip);
}

function pushShip(object) {
  totalShips++;
  genCoords(object);
  object.dir = getDir(ship, object);
  objects.push(object);
}
...
function drawBoost(object) {
  resetMatrix();
  translate(
    object.xPos - ship.xPos + width / 2,
    object.yPos - ship.yPos + height / 2
  );
...
function drawFlames(object) {
  ...
  if (object.flame.left) {
    triangle(
      -size / 3.5,
      (-flameSize * size) / 80 + size / 2,
      -size / 3.5,
      (flameSize * size) / 80 + size / 2,
      -size / 3.5 - ((flameSize * size) / 80) * 2,
      size / 2
    );
  }
...
function genPlanet() {
  ...
  genCoords(newPlanet);
  if (ring) {
    newPlanet.ringColor = randomColor();
  }
  if (style === "Crater") {
    generateCraters(newPlanet);
  }
  if (style === "Gas") {
    gasCoords(newPlanet);
  }
  return objects.push(newPlanet);
}

function generateCraters(object) {
  object.craters = [];
  for (i = 0; i < 5 + parseInt(object.size / 200); i++) {
    genCrater(object);
  }
}
...
function genCrater(object, attempt = 1) {
  ...
  distance =
    Math.random() *
    (dir > shadeAngle - PI || dir < shadeAngle - 2 * PI
      ? object.size / 4
      : object.size / 2 - size / 2);
  newCrater = {
    xPos: Math.cos(dir) * distance,
    yPos: Math.sin(dir) * distance,
    size: size
  };
  for (i = 0; i < object.craters.length; i++) {
    if (collision(newCrater, object.craters[i])) {
      return genCrater(object, attempt++);
    }
  }
...
function makeCrater(a, b) {
  ...
  for (i = 0; i < a.craters.length; i++) {
    if (collision(newCrater, a.craters[i])) {
      if (a.craters[i].size > size) {
        return;
      } else {
        a.craters.splice(i, 1);
      }
    }
  }
...
function gasCoords(object) {
  ...
  let a = [
    (object.size / 2) * Math.cos(angle1),
    (object.size / 2) * Math.sin(angle1)
  ];
  let b = [
    (object.size / 2) * Math.cos(angle2),
    (object.size / 2) * Math.sin(angle2)
  ];
  let c = [
    (object.size / 2) * Math.cos(angle3),
    (object.size / 2) * Math.sin(angle3)
  ];
  let d = [
    (object.size / 2) * Math.cos(angle4),
    (object.size / 2) * Math.sin(angle4)
  ];
  ...
  let e = [
    (object.size / 2) * Math.cos(angle5),
    (object.size / 2) * Math.sin(angle5)
  ];
  let f = [
    (object.size / 2) * Math.cos(angle6),
    (object.size / 2) * Math.sin(angle6)
  ];
  let g = [
    (object.size / 2) * Math.cos(angle7),
    (object.size / 2) * Math.sin(angle7)
  ];
  let h = [
    (object.size / 2) * Math.cos(angle8),
    (object.size / 2) * Math.sin(angle8)
  ];
...
function drawPlanet(object) {
  ...
  if (object.style === "Crater") {
    drawCraters(object);
  }
  if (object.ring) {
    drawRing(object, true);
  }
}
...
function drawPlanetShadow(object) {
  ...
  let midPoint = [
    (object.size / 2) * Math.sin(midAngle),
    (object.size / 2) * Math.cos(midAngle)
  ];
  let a = [
    start[0] +
      (object.size / 2) *
        (4 / 3) *
        Math.tan(PI / 8) *
        Math.sin(angle1 + PI / 2),
    start[1] +
      (object.size / 2) * (4 / 3) * Math.tan(PI / 8) * Math.cos(angle1 + PI / 2)
  ];
  let b = [
    midPoint[0] +
      (object.size / 2) *
        (4 / 3) *
        Math.tan(PI / 8) *
        Math.sin(midAngle - PI / 2),
    midPoint[1] +
      (object.size / 2) *
        (4 / 3) *
        Math.tan(PI / 8) *
        Math.cos(midAngle - PI / 2)
  ];
  let c = [
    midPoint[0] +
      (object.size / 2) *
  ...
  let d = [
      ...
      (object.size / 2) *
        (4 / 3) *
        Math.tan(PI / 8) *
        Math.sin(angle2 - PI / 2),
    end[1] +
      (object.size / 2) * (4 / 3) * Math.tan(PI / 8) * Math.cos(angle2 - PI / 2)
  ];
  fill(0, 100);
  beginShape();
...
function drawRing(object, front) {
  let ringSize =
    ((object.size / 2) * (width - (ship.yPos - object.yPos))) / width;
...
function drawCraters(object) {
  for (i = 0; i < object.craters.length; i++) {
    fill(object.color);
    ellipse(
      object.craters[i].xPos,
      object.craters[i].yPos,
      object.craters[i].size
    );
    fill(0, 200);
    ellipse(
      object.craters[i].xPos,
      object.craters[i].yPos,
      object.craters[i].size * 0.9
    );
    fill(object.color);
    ellipse(
      object.craters[i].xPos -
        Math.cos(shadeAngle + PI / 2) * object.craters[i].size * (1 / 18),
      object.craters[i].yPos -
        Math.sin(shadeAngle + PI / 2) * object.craters[i].size * (1 / 18),
      object.craters[i].size * 0.8
    );
    fill(0, 100);
    ellipse(
      object.craters[i].xPos -
...
function drawBullet(object) {
  ...
  if (object.missile) {
    ...
    ellipse(0, 0, object.size, object.size);
  }
}
